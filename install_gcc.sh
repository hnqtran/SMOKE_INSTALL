#!/bin/bash
# SMOKE Spack Deployment Engine - GCC
# Dedicated track for modern GCC hydration and SMOKE compilation.
...
if [[ "${1:-}" == smoke* && "${2:-}" == %* ]]; then
    COMPILER_SPEC="$1 $2"
    MY_INSTALL_ROOT="${3:-$PWD/install_gcc}"
else
    COMPILER_SPEC="${1:-%gcc}"
    MY_INSTALL_ROOT="${2:-$PWD/install_gcc}"
fi
...
generate_final_config() {
    log "Finalizing GCC foundation enrolment via discovery..."
    SPACK_GCC_PATH=$(spack find --format "{prefix}" gcc@14 | head -n 1)
    
    # Ensure production GCC is registered
...
rm -f smoke-gcc && ln -s "$CURRENT_SMOKE" smoke-gcc
log "Done. Shortcut: ./smoke-gcc/bin"
